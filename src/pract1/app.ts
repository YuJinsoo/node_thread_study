...
import * as jsonConfig from './env_file.json';
...
const env_config: Env = jsonConfig;
...
const t_resultArr: (number|string)[] = [];
const t_result_map = new Map<number, any>();
...
if (env_config.useThread) {
    for (let i = 0; i < env_config.numThreads; i++) {
        let myWorker = new Worker(worker_path);
